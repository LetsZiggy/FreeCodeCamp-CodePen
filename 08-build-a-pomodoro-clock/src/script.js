  ...
  if(mouseDown !== false) {
    ...
    if(Math.abs(diff) > 15) {
      ...
      if(mouseDown === 'break') {
        ...
        else if(breakVal >= 30) { breakVal = 30; }

        breakElem.innerText = `${breakVal.toString()} min${breakVal > 1 ? 's' : ''}`;
      }
      else if(mouseDown === 'pomodoro') {
        if(diff > 0) { ++pomodoroVal; } else { --pomodoroVal; }
        if(pomodoroVal <= 1) { pomodoroVal = 1; }
        else if(pomodoroVal >= 50) { pomodoroVal = 50; }

        pomodoroElem.innerText = `${pomodoroVal.toString()} cycle${pomodoroVal > 1 ? 's' : ''}`;
      }
      else if(mouseDown === 'session') {
        if(diff > 0) { ++sessionVal; } else { --sessionVal; }
        if(sessionVal <= 1) { sessionVal = 1; }
        else if(sessionVal >= 60) { sessionVal = 60; }
